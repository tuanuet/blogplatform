...
func TestAuthUseCase_Register(t *testing.T) {
	...
	authUC := usecase.NewAuthUseCase(mockUserRepo, mockSessionRepo, nil, nil)
...
func TestAuthUseCase_LoginWithSocial(t *testing.T) {
	...
	mockUserRepo := mocks.NewMockUserRepository(ctrl)
	mockSessionRepo := mocks.NewMockSessionRepository(ctrl)
	mockSocialRepo := NewMockSocialAccountRepository(ctrl)
	mockSocialAuthService := NewMockSocialAuthService(ctrl)

	authUC := usecase.NewAuthUseCase(mockUserRepo, mockSessionRepo, mockSocialRepo, mockSocialAuthService)
	ctx := context.Background()
